...
function script_aggro:closeToAdds()
	...
	while i ~= 0 do
		if t == 3 then
			...
			if i:GetDistance() <= 30 and not i:IsDead() and not i:IsCritter() and i:CanAttack() and script_grind:isTargetBlacklisted(i:GetGUID()) and not script_grind:isTargetHardBlacklisted(i:GetGUID()) and i:IsInLineOfSight() then
...
function script_aggro:avoidElite(range) -- Runs away if there is atleast one elite within range
	...
	while currentObj ~= 0 do
 		if typeObj == 3 and (currentObj:GetClassification() == 1 or currentObj:GetClassification() == 2) then
			if not script_grind:isTargetingMe(currentObj) and not script_grind:isTargetingPet(currentObj) and currentObj:CanAttack() and currentObj:GetDistance() <= range and not currentObj:IsDead() then	
