	...
	while currentObj ~= 0 do
		...
		if (typeObj == 3)  then
			...
			if (currentObj:CanAttack()) and (not currentObj:IsDead()) and (not currentObj:IsCritter()) and (currentObj:GetGUID() ~= GetLocalPlayer():GetGUID()) and (currentObj:GetLevel() >= GetLocalPlayer():GetLevel() - 4) and (not currentObj:IsCasting()) then
				...
				-- acceptable range
				if (GetDistance3D(tx, ty, tz, cx, cy, cz) - 1 <= aggro) then	
...
function script_aggro:safePullRecheck(target) 
	...
	while currentObj ~= 0 do
		...
		if (typeObj == 3) then
			...
			local testRange = GetDistance3D(tx, ty, tz, rx, ry, rz) - 1;
...
function script_aggro:drawAggroCircles(maxRange)
	...
	while currentObj ~= 0 do
		...
		-- acceptable targets
		if typeObj == 3 and not friendlyEnemiesList:isEnemyNuetral(currentObj) and currentObj:GetDistance() < maxRange and not currentObj:IsDead() and currentObj:CanAttack() and not currentObj:IsCritter() and currentObj:GetLevel() ~= 1 then
...
function script_aggro:closeToAdds()
	...
	while i ~= 0 do
		if t == 3 then
			...
			if i:GetDistance() <= 30 and not i:IsDead() and not i:IsCritter() and i:CanAttack() and script_grind:isTargetBlacklisted(i:GetGUID())
			and not script_grind:isTargetHardBlacklisted(i:GetGUID()) and i:IsInLineOfSight() and not script_aggro:safePullRecheck(i) then
