...
function script_aggro:drawAggroCircles(maxRange)
	...
	while currentObj ~= 0 do
		...
		-- acceptable targets
 		if typeObj == 3 and currentObj:GetDistance() < maxRange and not currentObj:IsDead() and currentObj:CanAttack() and not currentObj:IsCritter() and currentObj:GetLevel() ~= 1 then
...
function script_aggro:avoidElite() -- Runs away if there is atleast one elite within range
	...
	while currentObj ~= 0 do
 		if typeObj == 3 and (currentObj:GetClassification() == 1 or currentObj:GetClassification() == 2) then
			...
			range = currentObj:GetLevel() - localObj:GetLevel() + 27.5;
