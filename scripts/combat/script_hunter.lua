		...
		if not PlayerHasTarget() then
 		...
 		if (distance < range)  then
				...
				if script_grind.enemyObj ~= nil then
					if not script_grind.enemyObj:IsInLineOfSight() then
						PetFollow();
					else
						PetAttack();
					end
...
-- walk away from target if pet target guid is the same guid as target targeting me
	...
	and targetObj:GetUnitsTarget() ~= nil
		...
		then
			...
			if targetObj:GetDistance() < 14 then
				if (script_hunter:runBackwards(targetObj, 17)) then
					...
					if targetObj:IsInLineOfSight() then
						if GetTimeEX() > self.petAttackTimer then PetAttack(); self.petAttackTimer = GetTimeEX() + 1500; end;
					--elseif not targetObj:IsInLineOfSight() then
					--	if GetTimeEX() > script_hunter.petFollowTimer then PetFollow(); self.waitTimer = GetTimeEX() + 1000script_hunter.petFollowTimer = GetTimeEX() + 1500; end;
					end				
				return 4;
				end
			return true;
	...
	if HasPet() and not IsInCombat() then
...
-- set combat script attack distance
	if PlayerHasTarget() then
		...
		if (GetTarget():GetDistance() < 12 and script_grind:isTargetingMe(targetObj))
...
-- return and do nothing if we are channeling, casting or wait timer/tick rate, or we are stunned
	...
	if IsAutoCasting("Auto Shot") and not IsMoving() and targetObj:GetDistance() >= 13 then
		targetObj:FaceTarget();
	end
	...
	if (IsInCombat()) and (HasPet()) and (not PlayerHasTarget()) and (GetNumPartyMembers() < 1) and (self.hasPet) and not GetPet():IsDead() then
		if (PetHasTarget()) and not script_grind:isTargetingMe(GetPet():GetUnitsTarget()) then
			--if (GetPet():GetDistance() > 12) then
				AssistUnit("pet");
				if GetTimeEX() > script_hunter.petFollowTimer then PetFollow(); script_hunter.petFollowTimer = GetTimeEX() + 1500; end;
			--end
...
-- target not in line of sight call pet
	if (GetPet() ~= 0) then
...
-- we have a valid enemy
	if (targetObj ~= 0 and targetObj ~= nil) then
		...
		and not targetObj:IsDead() and targetObj:CanAttack() then
			...
			if (not IsAutoCasting("Auto Shot")) and not targetObj:IsDead() and targetObj:IsInLineOfSight() then
		...
		if IsInCombat() and HasPet() and GetTimeEX() > self.petAttackTimer - 1000
...
-- NOT in combat ---  do pull stuff
		...
		if (GetLocalPlayer():GetLevel() < 12) then
			if (targetObj:GetDistance() > 12) and (targetObj:GetDistance() < self.spellRange) and targetObj:IsInLineOfSight() and self.useRangedAttacks then
				script_hunter:hunterPull(targetObj);

				-- else move to target to melee
			elseif (targetObj:GetDistance() < 12) or not targetObj:IsInLineOfSight() or not self.useRangedAttacks then
...
-- if we are not in combat then do pull ELSE
		if (not IsInCombat()) and (targetObj:GetDistance() < self.spellRange) and (targetObj:GetDistance() > 12) and self.useRangedAttacks
...
-- force auto shot if in combat
			...
			if (self.hasPet) and HasPet() and (petHP < 1 or GetPet():IsDead()) then
				if (HasSpell("Feign Death")) and (not IsSpellOnCD("Feign Death"))
				and (localMana > 7) and (script_grind:enemiesAttackingUs() > 1) then
...
-- target is far enough to use ranged attacks
			...
			if (targetObj:GetDistance() > 12) and (targetObj:GetDistance() < self.spellRange) and self.useRangedAttacks then
				...
				-- use arcane shot
				if (not IsSpellOnCD("Arcane Shot")) and localMana >= self.arcaneShotMana then
...
function script_hunter:rest()
	...
	if (not IsInCombat()) and (self.feedTimer > GetTimeEX()) and (self.useFeedPet) and (not IsInCombat())
...
function script_hunter:hunterPull(targetObj)
	...
	if (self.hasPet) and (not IsMoving()) and targetObj:IsInLineOfSight()
	and not targetObj:IsDead() and targetObj:CanAttack()
	then
		PetAttack();
	end
	...
	and (IsStanding()) and (targetObj:GetDistance() > 12)
	and not targetObj:IsDead() then
		if (not IsMoving()) and targetObj:GetDistance() >= 13 and targetObj:GetDistance () <= self.spellRange then
			targetObj:FaceTarget();
		end
						...
						if GetPet():GetUnitsTarget():GetGUID() ~= i:GetGUID() then
							...
							if PlayerHasTarget() then
								...
								if GetLocalPlayer():GetUnitsTarget():GetGUID() ~= i:GetGUID() then
									...
									-- target the target
									if i:GetGUID() ~= script_grind.enemyObj:GetGUID() then
										i:UnitInteract();
									end
