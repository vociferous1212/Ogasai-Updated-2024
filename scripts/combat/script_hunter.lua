...
-- Check: Do we have a pet?
	...
	-- walk away from target if pet target guid is the same guid as target targeting me or has wingclip or has concussive shot
	if targetObj ~= nil
	and targetObj ~= 0
	and GetPet() ~= 0
	and GetPet() ~= nil
	...
	and ( not script_grind:isTargetingMe(targetObj) or (targetObj:HasDebuff("Wing Clip") or targetObj:HasDebuff("Concussive Shot")) )
	...
	then
		...
		or (targetObj:HasDebuff("Wing Clip") or targetObj:HasDebuff("Concussive Shot"))
...
-- return and do nothing if we are channeling, casting or wait timer/tick rate, or we are stunned
	...
	if IsAutoCasting("Auto Shot") and not IsMoving() then
...
-- melee attacks otherwise
			...
			if (targetObj:GetDistance() < 12) then
				...
				and (targetObj:GetDistance() <= self.meleeDistance) and not HasPet() then
					if (not IsMoving()) then
						targetObj:FaceTarget();
					end
...
function script_hunter:hunterPull(targetObj)
	...
	if (not IsSpellOnCD("Concussive Shot")) and (IsStanding()) and (not HasPet() or not self.hasPet) then
