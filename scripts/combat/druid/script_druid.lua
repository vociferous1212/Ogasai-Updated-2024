...
function script_druid:run(targetGUID)
	...
	if localObj:HasBuff("Nature's Grasp") and IsInCombat() then return 4; end
		...
		if IsInCombat() and not HasForm() and localHealth <= self.healthToShift and GetLocalPlayer():HasBuff("Regrowth") then
			if not IsCasting() and not IsChanneling() and localMana >= 25 then
...
-- use charge in bear form
	...
	-- shapeshift out of cat form to use bear form 2 or more targets - leave form
		...
		if (IsInCombat()) and (script_grind.enemiesAttackingUs() == 1) and (not IsCatForm()) and (self.useCat) and (not self.useBear) and (not IsBearForm()) and (localHealth >= self.healthToShift + 10 or ( (hasRegrowth and hasRejuv) or not hasRejuv and not HasSpell("Regrowth")) ) and (localMana >= self.shapeshiftMana) and (targetObj:GetLevel() <= localObj:GetLevel() +2) and (not IsDrinking()) and (not IsEating()) then
	...
	-- attacks in cat form IN COMBAT PHASE
		...
		-- Ferocious Bite with 5 CPs
				...
				if IsCatForm() and (targetHealth - 10 <= (10 * localCP)) and (localEnergy >= 35) and (HasSpell("Ferocious Bite")) and not IsSpellOnCD("Ferocious Bite") then
					if (CastSpellByName("Ferocious Bite", targetObj)) then
						self.waitTimer = GetTimeEX() + 1000;
						return;
					end
				...
				-- Use Claw
				if IsCatForm() and (localEnergy >= self.clawEnergy) and (not IsSpellOnCD("Claw")) and not IsCasting() and not IsChanneling() then
