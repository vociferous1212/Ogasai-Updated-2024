...
function script_druidHealsAndBuffs:healsAndBuffs()
	...
	-- set mana, rage and energy if we have clearcasting buff to 100
	if player:HasBuff("Clearcasting") then
		localMana = 100;
		localRage = 100;
		localEnergy = 100;
	-- else get mana, rage and energy
	else
		localMana = player:GetManaPercentage();
		localRage = player:GetRagePercentage();
		localEnergy = player:GetEnergyPercentage();
	end
	...
	-- set combat script range
	if (script_druid.useBear or script_druid.useCat) or (not script_druid.useBear and not script_druid.useCat and localMana <= 30 and IsInCombat()) then
		script_druid.spellRange = script_druid.meleeDistance;
	elseif not script_druid.useBear and not script_druid.useCat and localMana >= 30 then
		script_druid.spellRange = 27;
	end
	
	script_grind.combatScriptRange = script_druid.spellRange;
	...
	if (PlayerHasTarget()) then
		if (IsBearForm()) and (HasSpell("Bash")) and (not IsSpellOnCD("Bash")) and (localRage >= 10) and (myTarget:GetDistance() <= script_druid.meleeDistance) and (targetHealth >= 15) and (localHealth <=script_druid.healthToShift) then
...
--[[
-- return until nature's grasp is used
	if localObj:HasBuff("Nature's Grasp") and IsInCombat() then
		return false;
	end
	...
	if IsInCombat() and HasSpell("Nature's Grasp") and not IsSpellOnCD("Nature's Grasp") and not HasForm() and not IsIndoors() and localHealth <= 55 and GetTimeEX() > script_druid.naturesGraspTimer then
		...
		script_druid.naturesGraspTimer = GetTimeEX() + 60000;
		return true;
	end
...
--]]
			...
			if (localHealth <= script_druid.regrowthHealth) and (localMana >= 40) and (not localObj:HasBuff("Regrowth")) then
				if (not IsMoving()) and (not IsLooting()) and (not localObj:HasBuff("Regrowth")) then	
					...
					if (not script_druid.hasRegrowth) then
						...
						script_druid.hasRegrowth = true;
						if not CastSpellByName("Regrowth", localObj) then
							script_druid.waitTimer = GetTimeEX() + 550
	...
	if not IsSpellOnCD("Healing Touch") and (not HasSpell("Regrowth")) and (not IsBearForm() and not IsCatForm() and not isMoonkin and not IsTravelForm()
	and not IsMounted()) and (localHealth < script_druid.healthToShift) and (not IsSpellOnCD("Healing Touch")) and (not script_checkDebuffs:hasSilence()) then
		if (not IsCasting()) and (not IsChanneling()) then
