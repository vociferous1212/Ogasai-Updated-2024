...
function script_paladinHealsAndBuffs:healsAndBuffs()
	...
	if (localObj:HasBuff("Divine Shield") or localObj:HasBuff("Divine Protection") or localObj:HasBuff("Blessing of Protection")) then
		if (localMana > 15) then
			...
			if (localMana > 8) and (HasSpell("Flash of Light")) and not IsSpellOnCD("Flash of Light") then
				if (IsMoving()) then
					StopMoving();
				end
	...
	-- cleanse
	if (script_checkDebuffs:hasPoison()) or (script_checkDebuffs:hasDisease()) or (script_checkDebuffs:hasMagic()) and not IsSpellOnCD("Cleanse") then
	...
	if not IsInCombat() and localMana > 10 and HasSpell("Flash of Light") and localHealth <= 80 and not IsSpellOnCD("Flash of Light") then
	...
	if (script_paladin.useFlashOfLightInCombat) and localMana >= 7 and localHealth <= script_paladin.flashOfLightHealth and not IsSpellOnCD("Flash of Light") then
	...
	if (HasSpell("Flash of Light")) and (IsInCombat()) and (localMana < 15) and (localMana > 5) and (localHealth < script_paladin.holyLightHealth) and not IsSpellOnCD("Flash of Light") then
