...
-- set combat script attack distance
	if PlayerHasTarget() then
		...
		if (GetTarget():GetDistance() < 6 and script_grind:isTargetingMe(targetObj)) then
	...
	if (HasSpell("Summon Imp")) and (not HasPet()) and (self.useImp or self.useVoid or self.useSuccubus or self.usefelHunter) then	
		...
		if (not HasPet()) and (not IsCasting()) and (not IsChanneling()) and (not localObj:HasBuff("Blood Pact")) and not IsMoving() then
...
-- Opener check range of ALL SPELLS
		...
		if targetObj:GetDistance() <= 4 and not IsMoving() then
