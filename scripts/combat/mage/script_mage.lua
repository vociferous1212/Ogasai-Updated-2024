...
function script_mage:run(targetGUID)
	...
	if self.startedNewCharacter then
		if HasSpell("Frostbolt") and self.fireMage then
			self.spellRange = 29;
			self.fireMage = false;
			self.frostMage = true;
	...
	-- Check: Move backwards if the target is affected by Frost Nova or Frost Bite
		-- run backwards
		if (GetNumPartyMembers() < 1) and (self.useFrostNova) and not IsCasting() and not IsChanneling() then
			if (targetObj:HasDebuff("Frostbite") or targetObj:HasDebuff("Frost Nova"))
			and (targetHealth > self.useWandHealth or localHealth < 35)
...
-- frost ward
		...
		-- Combat
			...
			if (self.useConeOfCold) and (HasSpell("Cone of Cold")) and (localMana >= self.coneOfColdMana)
			and (targetHealth >= self.coneOfColdHealth) and (IsSpellOnCD("Frost Nova") or script_grind:enemiesAttackingUs(10) >= 2) then
				if (not self.addPolymorphed) and (targetObj:GetDistance() < 9)
				and (not targetObj:HasDebuff("Frostbite")) and (not targetObj:HasDebuff("Frost Nova"))
				and not script_mage.addPolymorphed
				then
						targetObj:FaceTarget();
...
-- we only really want to use it as a last resort once we have better spells... costs a lot of mana
			...
			if (self.useWand)
			and (localObj:HasRangedWeapon())
			and (localMana <= self.useWandMana or targetHealth <= self.useWandHealth)
			and (not IsChanneling())
			and (not localObj:IsStunned())
			and (not IsMoving())
			...
			-- low level fireball
			if (self.frostMage) and (not HasSpell("Frostbolt")) then
