script_mage = {
	...
	useShield = true,	-- use mana shield yes/no
	...
	shieldHealth = 80,	-- use mana shield below this health %
	shieldMana = 20,	-- use mana shield above this mana %
...
function script_mage:runBackwards(targetObj, range) 
	...
	-- if we don't check for LoS then the bot won't break the return 4 loop if the target leaves LoS
...
-- Assign the target 
	...
	if targetObj == 0 or targetObj == nil then return; end
...
-- Check: Move backwards if the target is affected by Frost Nova or Frost Bite
	...
	if (GetNumPartyMembers() < 1) and (script_mage.useFrostNova) and not IsCasting() and not IsChanneling() then
		...
		and (targetHealth >= script_mage.useWandHealth or localHealth < 35 or (targetHealth >= 20 and script_mage.useWandHealth >= 40))
		...
		and targetObj:IsInLineOfSight() then
			...
			if targetObj:GetDistance() < 10 then
				if (script_mage:runBackwards(targetObj, 11)) then -- Moves if the target is closer than 7 yards
...
-- check silence and use wand
	...
	then
		if (script_checkDebuffs:hasSilence()) and (not IsMoving())
		then
			if (targetObj ~= 0)
				and (targetObj ~= nil)
			then
...
-- frost ward
		...
		if (not IsInCombat()) then
			...
			if script_mage.useShield and localMana >= script_mage.drinkMana + 10 and localMana >= self.shieldMana and localHealth <= self.shieldHealth and PlayerHasTarget() and (HasSpell("Ice Barrier")) and (not IsSpellOnCD("Ice Barrier")) and (not localObj:HasBuff("Ice Barrier")) then
		...
		-- Combat
			...
			if (script_mage.useShield or localHealth <= 20) and localMana >= self.shieldMana and localHealth <= self.shieldHealth and (HasSpell("Ice Barrier")) and (not IsSpellOnCD("Ice Barrier")) and (not localObj:HasBuff("Ice Barrier")) then
				CastSpellByName('Ice Barrier');
			...
			if IsInCombat() and not script_checkDebuffs:hasDisabledMovement() and script_grind:enemiesAttackingUs(10) >= 2 then script_mageCheckFrostNova:checkFrostNova(); script_mage.waitTimer = GetTimeEX() + 500; end
...
-- we only really want to use it as a last resort once we have better spells... costs a lot of mana
			...
			if (script_mage.useshield or localHealth <= 20) and (not localObj:HasBuff("Ice Barrier")) and (HasSpell("Mana Shield")) and (localMana >= script_mage.shieldMana) and (localHealth <= script_mage.shieldHealth) and (not localObj:HasBuff("Mana Shield")) and (IsInCombat()) and (targetHealth >= 10 or script_grind:enemiesAttackingUs() >= 2 or localHealth <= 20) and targetObj:GetDistance() <= 12 then
			...
			if (HasSpell("Frostbolt")) and (script_mage.frostMage) and (not IsChanneling()) and (not IsMoving()) then
				if (localMana >= script_mage.useWandMana and targetHealth >= script_mage.useWandHealth) then
					...
					if localMana >= script_mage.frostboltMana and ((cost ~= nil and PlayerManaTotal() >= cost and cost ~= 0) or PlayerMana >= 10) and (not IsMoving()) and (not IsSpellOnCD("Frostbolt")) and not targetObj:IsInLineOfSight() and (targetObj:HasDebuff("Frostbite") or targetObj:HasDebuff("Frost Nova")) then
					...
					if localMana >= script_mage.frostboltMana and ((cost ~= nil and PlayerManaTotal() >= cost and cost ~= 0) or PlayerMana >= 10) and (not IsMoving()) and (not IsSpellOnCD("Frostbolt")) and targetObj:IsInLineOfSight() then
			...
			if (script_mage.frostMage) and (not IsMoving()) and (not localObj:HasRangedWeapon()) and (targetHealth <= script_mage.useWandHealth) and (not IsSpellOnCD("Frostbolt")) then				
				...
				-- cast frostbolt
				if targetObj:IsInLineOfSight() and not IsMoving() and ((cost ~= nil and PlayerManaTotal() >= cost and cost ~= 0) or PlayerMana >= 10) then
					if (CastSpellByName("Frostbolt", targetObj)) then
						script_grind:setWaitTimer(1650);
						script_mage.waitTimer = GetTimeEX() + 1650;
