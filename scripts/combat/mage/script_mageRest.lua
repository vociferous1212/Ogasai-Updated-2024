...
function script_mageRest:rest()
	...
	if (script_mage.waitTimer > GetTimeEX()) then
		return;
	end
...
if (not IsDrinking() and localMana < script_mage.drinkMana) and (not IsSwimming()) then if IsMoving() then StopMoving(); return true; end end
	...
	if (not IsEating()) and (not IsDrinking()) then
		...
		if script_mage.useFrostArmor and (HasSpell("Ice Armor")) and (not localObj:HasBuff("Ice Armor")) and (localMana > 20) then
...
-- eat AND drink....
	if (localHealth <= script_mage.eatHealth or (IsEating() and not IsDrinking() and localMana <= 85)) or (localMana <= script_mage.drinkMana or (IsDrinking() and not IsEating() and localHealth <= 80)) and not IsSwimming() then
		if IsMoving() then StopMoving(); return true; end
		...
		-- if we need to drink
		if (not IsDrinking() and localMana <= script_mage.drinkMana)
		
		-- if we are eating and we are not drinking then we should drink
		or (IsEating() and localMana <= 85 and not IsDrinking()) then
	...
	if (localMana < script_mage.drinkMana or localHealth < script_mage.eatHealth) then
		if (IsMoving()) then
			StopMoving();
			return true;
		end
