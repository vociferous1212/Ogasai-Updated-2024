...
function script_mageEX:menu()
	...
	if (script_mage.frostMage) or (script_mage.fireMage) then
		...
		if (CollapsingHeader("Mage Combat Options")) then
			...
			Text('Use Mana Potion / Mana Gem Below Percentage');
			...
			if (HasSpell("Mana Shield")) then
				...
				wasClicked, script_mage.useShield = Checkbox("Use Shield / Ice Barrier", script_mage.useShield);
			...
			if (script_mage.useShield) then

				if (HasSpell("Mana Shield")) then

					if (CollapsingHeader("|+| Shield Options")) then

						Text('Shield below script_mage health percent');
						script_mage.shieldHealth = SliderInt("MS%", 5, 99, script_mage.shieldHealth);

						Text('Shield above script_mage mana percent');
						script_mage.shieldMana = SliderInt("MM%", 10, 65, script_mage.shieldMana);
