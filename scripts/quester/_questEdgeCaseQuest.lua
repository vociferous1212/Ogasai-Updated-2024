	...
	if isComplete ~= 1 and title == "Lazy Peons" then
		...
		elseif target == nil then
			...
			elseif not self.targetReached then		
				grind2MoveToTarget:run(GetLocalPlayer(), -236.66152954102, -4433.4741210938, 63.312015533447);
			end
			...
			if self.targetReached then
				...
				grind2MoveToTarget:run(GetLocalPlayer(), -737.55145263672, -4168.05859375, 30.284696578979);
			end
	...
	if isComplete ~= 1 and title == "Practical Prey" then
		...
		elseif target == nil then
			...
			if GetDistance3D(x, y, z, -1382.1340332031, -5183.8759765625, 2.2133538722992) <= 10 then	
				self.targetReached = true;
			elseif not self.targetReached then		
				grind2MoveToTarget:run(GetLocalPlayer(), -1382.1340332031, -5183.8759765625, 2.2133538722992);
			end
			...
			if self.targetReached then
				...
				grind2MoveToTarget:run(GetLocalPlayer(), -1126.0758056641, -5127.5122070313, 2.9460308551788);
			end
