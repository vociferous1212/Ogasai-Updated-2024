...
function script_moveToHotspot:moveToHotspot(localObj)
	...
	if (script_nav.currentHotSpotName ~= 0) and (not script_checkDebuffs:hasDisabledMovement()) and (script_nav.numSavedLocation < 3 or not script_grind.hotspotReached or not script_grindEX:isThereAnyValidEnemyNearby()) then
		...
		if IsStanding() and not IsMounted() and HasSpell("Blink") and not IsSpellOnCD("Blink") and GetLocalPlayer():GetManaPercentage() >= script_grind.drinkMana + 20 then
				CastSpellByName("Blink");
				script_grind.tickRate = 50;
			end
		
		-- stop moving so we can mount
		if not IsSwimming() and GetTimeEX() > script_grind.tryMountTimer and (not IsInCombat()) and (not IsMounted()) and (not IsIndoors()) and (not HasForm()) and (script_grind.useMount and script_grind.hasAMount) then
			if (IsMoving()) then
				StopMoving();
				return;
			end
