...
function script_grindEX:howManyEnemiesTargetingMe()
	...
	while i ~= 0 do
		...
		if t == 3 or t == 4 then
			...
			if i:GetDistance() <= 40 and i:CanAttack() then
...
if script_grind.lastTargetTargeted ~= 0 and script_grind.lastTargetTargeted ~= nil and script_grind.lastTarget ~= 0 and script_grind.lastTarget ~= nil and script_grind.hotspotReached then
	...
	if (not IsInCombat() or IsMounted()) and (script_nav:getDistanceToHotspot() > script_grind.distToHotSpot) and (script_grind.hotspotReached) then
...
function script_grindEX:getTargetWithinAggroRange()
	...
	while i ~= 0 do
		if t == 3 or t == 4 then
			...
			if i:GetDistance() <= aggro and i:CanAttack() then
				return i;
			end
		end	
	i, t = GetNextObject(i);
