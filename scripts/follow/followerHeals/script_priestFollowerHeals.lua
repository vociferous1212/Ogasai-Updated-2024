...
function script_priestFollowerHeals:HealsAndBuffs()
	...
	if (GetTimeEX() > self.timer) then
		...
		if (script_followNumEnemiesAttackingUs:numEnemiesAttackingUs() >= 1) then
		...
		for i = 1, GetNumPartyMembers() do
			...
			if (GetNumPartyMembers() > 0) and (partyMember:GetGUID() ~= GetLocalPlayer():GetGUID()) then
				...
				-- priest fear
				if (script_followNumEnemiesAttackingUs:numEnemiesAttackingUs() > 3) and (HasSpell("Psychic Scream")) then
