...
function script_paladinFollowerHeals:HealsAndBuffs()
	...
	if (script_followNumEnemiesAttackingUs:enemiesAttackingUs() >= 2) then
			local localMana = GetLocalPlayer():GetManaPercentage();
			...
			local px, py, pz = GetPartyMember(i):GetPosition();
					...
					if (not member:HasBuff("Strength of Earth")) and (not member:HasBuff("Mana Spring")) then
						...
						or (member:HasBuff("Bear Form") or member:HasBuff("Cat Form")) and (not member:GetManaPercentage() > 1) then
							...
							and leaderObj:IsInLineOfSight()) then 															MoveToTarget(px, py, pz);
					...
					if (localMana > 5) and (membersHP < self.bopHealth) and (HasSpell("Blessing of Protection")) then
							if (not member:IsInLineOfSight() and memberDistance < script_follow.followLeaderDistance and leaderObj:IsInLineOfSight()) then 				MoveToTarget(px, py, pz);
							self.waitTimer = GetTimeEX() + 550;
							script_follow.waitTimer = GetTimeEX() + 550;
							return true;
								end
