   		...
   		for i = 1, GetNumPartyMembers() do
			...
			if (GetNumPartyMembers() > 0) then
				...
				if (HasSpell("Unending Breath")) and (script_warlock.useUnendingBreath) and (not partyMember:HasBuff("Unending Breath")) then
 					if (not partyMember:IsInLineOfSight() and partyMemberDistance() < script_follow.followLeaderDistance and leaderObj:IsInLineOfSight()) then 											MoveToTarget(px, py, pz);
							return true;
                       			end
