	...
	while i ~= 0 do
		if t == 4 then
			if (i:GetDistance() < 28) and (not i:HasBuff(buffCheck)) and (not i:HasBuff(buff2Check)) and (not i:CanAttack()) and (i:IsInLineOfSight()) then
				if (HasSpell(buff2)) and (i:GetManaPercentage() ~= nil and i:GetManaPercentage() > 1) then	
					for r=6, 1, -1 do 
						if i:GetLevel() >= Sp[r] then
							i:AutoAttack();
							local myTarget = i;
				...
				elseif ( (HasSpell(buff)) and (i:GetManaPercentage() == nil or i:GetManaPercentage() < 1) or (i:GetRagePercentage() ~= nil or i:GetRagePercentage() < 1) or (i:GetEnergyPercentage() ~= nil or i:GetEnergyPercentage() > 1)) or (not HasSpell(buff2)) then
					for r=6, 1, -1 do 
						if i:GetLevel() >= Sp[r] then
							i:AutoAttack();
							local myTarget = i;
