...
local target = 0; local target2 = 0; local numKill = 0; local numKill2 = 0; local i, t = GetFirstObject(); local dist = 0; local bestDist = 1000; local bestTarget = nil;
	...
	while i ~= 0 do
		if t == 3 then
			if not i:IsDead() and ((i:GetUnitName() == target and _quest.targetKilledNum < numKill) or (i:GetUnitName() == target2 and _quest.targetKilledNum2 < numKill2) or (i:GetUnitName() == target3 and _quest.targetKilledNum3 < numKill3)) then
				dist = i:GetDistance();
...
bestDist = dist;
	...
	if bestTarget == nil then _quest.message = "No quest targets in range!"; elseif bestTarget ~= nil then bestTarget:AutoAttack(); end return bestTarget; end