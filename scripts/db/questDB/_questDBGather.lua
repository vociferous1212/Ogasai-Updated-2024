...
function _questDBGather:run()
    ...
    if self.gatheringTarget ~= 0 and self.gatheringTarget ~= nil then
        -- Check for enemies near the gather object
        local i, t = GetFirstObject();
        _quest.enemyTarget = 0; -- Reset _quest.enemyTarget
        while i ~= 0 do
            if t == 3 then
                if not i:IsDead() and not i:IsCritter() and i:CanAttack() then
                    local nx, ny, nz = self.gatheringTarget:GetPosition();
                    local ix, iy, iz = i:GetPosition();
                    local dist = GetDistance3D(nx, ny, nz, ix, iy, iz);
                    local aggro = i:GetLevel() - GetLocalPlayer():GetLevel() + 22.2;
                    if dist <= aggro then
                        _quest.enemyTarget = i; -- Set the enemy as _quest.enemyTarget
