...
local target = 0; local target2 = 0; local numKill = 0; local numKill2 = 0; local i, t = GetFirstObject(); local dist = 0; local bestDist = 1000; local bestTarget = nil;
	...
	if not self.weHaveQuestTarget then
		...
		while i ~= 0 do
			if t == 3 and not _questDBTargets:isItTapped() and not i:IsDead() and not i:IsCritter() and i:CanAttack() and script_aggro:safePullRecheck(i) then
				if script_grindValidEnemy:enemyIsValid(i) and i:GetDistance() <= 200 then
					dist = i:GetDistance();
					if bestDist > dist then
						bestDist = dist;
						bestTarget = i;
	...
	if not IsInCombat() then
		...
		while i ~= 0 do
			-- acceptable targets
			if t == 3 then
			if not _questDBTargets:isItTapped() and not i:IsCritter() and not i:IsDead() and i:CanAttack() and i:GetDistance() <= 30 and script_grindValidEnemy:enemyIsValid(i) then
	...
	if bestTarget == nil then _quest.message = "No quest targets in range!"; self.weHaveQuestTarget = false; if not _quest.needRest then script_navEX:moveToTarget(GetLocalPlayer(), _quest.curGrindX, _quest.curGrindY, _quest.curGrindZ); script_grindAssignTarget:assignTarget(); end
