...
function script_helper:useMount()
	...
	if script_grind.waitTimer > GetTimeEX() then return false; end
			script_grind.tryMountTimer = GetTimeEX() + 10000;
...
function script_helper:mountUp()
	...
	if (GetLocalPlayer():GetLevel() >= 40) and (not IsInCombat()) and (not HasForm())
