...
function grind2SelectTalents:setup()
	...
	if (not self.isSetup) then
		...
		if (class == "PALADIN") then		self.currentTalentTable = self.palaTalents;
		elseif (class == "WARRIOR") then	self.currentTalentTable = self.warTalents;
		elseif (class == "ROGUE") then		self.currentTalentTable = self.rogueTalents;
		elseif (class == "DRUID") then		self.currentTalentTable = self.druidTalents;
		elseif (class == "PRIEST") then		self.currentTalentTable = self.priestTalents;
		elseif (class == "MAGE") then		self.currentTalentTable = self.mageTalents;
		elseif (class == "HUNTER") then		self.currentTalentTable = self.hunterTalents;
		elseif (class == "WARLOCK") then	self.currentTalentTable = self.lockTalents;
		elseif (class == "SHAMAN") then		self.currentTalentTable = self.shamTalents;
		end
...
function grind2SelectTalents:learnTalents()
	...
	if (self.talentTimer < GetTimeEX()) then
		...
		-- Spend talent points
		grind2SelectTalents:learnClassTalents();

		self.talentTimer = GetTimeEX() + 750;
...
function grind2SelectTalents:getMaxRank(talentName)
	...
	for i = 1, 61 do
		...
		if self.currentTalentTable[self.currentTalentPoint] ~= nil then
			...
			if self.currentTalentTable[i] ~= nil then
				...
				currentTalent = grind2SelectTalents:stringSplit(self.currentTalentTable[i]);
...
function grind2SelectTalents:learnTalent(tabIndex, talentName)
	
	local talentIndex = grind2SelectTalents:getTalentIndex(tabIndex, talentName);
...
function grind2SelectTalents:getTalentIndex(tabIndex, talentName)
	...
	for i = 1, numTalents do
		...
		if (name == talentName) then
			...
			if (rank ~= nil) then
				...
				if (rank < grind2SelectTalents:getMaxRank(talentName)) then
...
function grind2SelectTalents:getNextTalentName()
	...
	if self.currentTalentTable[self.currentTalentPoint] ~= nil then
		...
		-- Split the talent-string into the talent tab number and the talent name
		currentTalent = grind2SelectTalents:stringSplit(self.currentTalentTable[self.currentTalentPoint]);
...
function grind2SelectTalents:learnClassTalents()
	...
	if tabIndex ~= nil and talentName ~= nil then
		...
		grind2SelectTalents:learnTalent(tabIndex, talentName);
	end
...
function grind2SelectTalents:stringSplit(str)
