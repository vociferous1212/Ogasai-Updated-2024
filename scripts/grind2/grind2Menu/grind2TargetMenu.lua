...
function grind2TargetMenu:run()
	...
	if CollapsingHeader("Target Options") then
		...
		grind2.findTargetDistance = SliderInt("Find Target Distnce", 1, 250, grind2.findTargetDistance);

		Separator();

		Text("Min Target Level");
		grind2IsTargetValid.minLevel = SliderInt("Min Target Level", 1, 62, grind2IsTargetValid.minLevel);
		Separator();
		Text("Max Target Level");
		grind2IsTargetValid.maxLevel = SliderInt("Max Target Level", 1, 62, grind2IsTargetValid.maxLevel);
