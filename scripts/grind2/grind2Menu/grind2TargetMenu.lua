...
function grind2TargetMenu:run()
	...
	if CollapsingHeader("Target Menu") then
		...
		Separator();
		Text("Level"); SameLine(); grind2IsTargetValid.minLevel = SliderInt("Min level", 1, 62, grind2IsTargetValid.minLevel);
		Text("Level"); SameLine(); grind2IsTargetValid.maxLevel = SliderInt("Max level", 1, 62, grind2IsTargetValid.maxLevel);
